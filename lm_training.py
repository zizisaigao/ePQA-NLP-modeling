...
class TransformerLM(nn.Module):
    ...
    def encode(self, input_ids: torch.Tensor, attention_mask=None):
        """
        Return last hidden states before vocab projection.
        Accepts input_ids as [B,T] (Part3) or [T,B] (LM training style).
        Returns hidden as [B,T,D] for downstream pooling.
        """
        if input_ids.dim() != 2:
            raise ValueError(f"encode expects 2D tensor, got {input_ids.shape}")

        # If batch-first [B,T], transpose to [T,B]
        if input_ids.size(0) != input_ids.size(1) and input_ids.shape[0] < input_ids.shape[1]:
            # ambiguous heuristic; safer: detect by typical batch size
            # We'll explicitly handle both: if looks like [B,T] then transpose
            x = input_ids.transpose(0, 1)  # [T,B]
        else:
            x = input_ids  # assume [T,B]
    ...
    def forward(self, x: torch.Tensor):
        """
        For LM: returns logits. Supports x as [T,B] (original) or [B,T] (optional).
        """
        hidden_bt = self.encode(x)          # [B,T,D]
        hidden_tb = hidden_bt.transpose(0, 1)  # [T,B,D]
        logits = self.fc(hidden_tb)         # [T,B,V]
        return logits
...
def train_neural(
    ...
    device: torch.device,
    log_every: int = 200,
    epoch_log_jsonl: str = None,   # NEW: optional jsonl path
) -> Dict[str, object]:
    ...
    for ep in range(1, epochs + 1):
        ...
        # NEW: grad-norm stats (clip_grad_norm_ returns total norm BEFORE clipping)
        grad_norm_sum = 0.0
        grad_norm_max = 0.0
        ...
        tok_per_sec = total_tokens / max(1e-9, epoch_time)
        grad_norm_mean = grad_norm_sum / max(1, steps)

        print(f"[epoch] ep={ep} train_ppl={train_ppl:.2f} dev_ppl={dev_ppl:.2f} "
              f"time={epoch_time:.1f}s lr={lr_now:g} tok/s={tok_per_sec:.1f} "
              f"gn_mean={grad_norm_mean:.3f} gn_max={grad_norm_max:.3f}")
        ...
        # NEW: optional write jsonl (append)
        if epoch_log_jsonl:
            with open(epoch_log_jsonl, "a", encoding="utf-8") as f:
...
def main():
    ...
    ap.add_argument("--outdir", default="result_4_dim300", help="output directory")
    ...
    t0 = time.time()
    rnn_logs = train_neural(rnn, train_data, dev_data, args.seq_len, args.epochs, args.lr_rnn, args.clip, device)
    ...
    results["rnn"] = {
        ...
        "best_epoch": rnn_logs["best_epoch"],
        "best_dev_ppl": rnn_logs["best_dev_ppl"]
    }

    torch.save(rnn.state_dict(), outdir / "rnn.pt")

    with open(outdir / "rnn_history.json", "w", encoding="utf-8") as f:
        json.dump(rnn_logs, f, indent=2)

    print(f"[rnn] dev_ppl={dev_ppl:.2f} test_ppl={test_ppl:.2f} train_time={tr_time:.1f}s best_ep={rnn_logs['best_epoch']}")
    ...
    t0 = time.time()
    lstm_logs = train_neural(lstm, train_data, dev_data, args.seq_len, args.epochs, args.lr_rnn, args.clip, device)
    tr_time = time.time() - t0
    ...
    results["lstm"] = {
        ...
        "test_ppl": test_ppl,
        "train_time_s": tr_time,
        "best_epoch": lstm_logs["best_epoch"],
        "best_dev_ppl": lstm_logs["best_dev_ppl"],
    }

    torch.save(lstm.state_dict(), outdir / "lstm.pt")

    with open(outdir / "lstm_history.json", "w", encoding="utf-8") as f:
        json.dump(lstm_logs, f, indent=2)

    print(f"[lstm] dev_ppl={dev_ppl:.2f} test_ppl={test_ppl:.2f} train_time={tr_time:.1f}s best_ep={lstm_logs['best_epoch']}")
    ...
    t0 = time.time()
    tf_logs = train_neural(tf, train_data, dev_data, args.seq_len, args.epochs, args.lr_tf, args.clip, device)
    tr_time = time.time() - t0
    ...
    results["transformer"] = {
        ...
        "test_ppl": test_ppl,
        "train_time_s": tr_time,
        "best_epoch": tf_logs["best_epoch"],
        "best_dev_ppl": tf_logs["best_dev_ppl"],
    }

    torch.save(tf.state_dict(), outdir / "transformer.pt")

    with open(outdir / "transformer_history.json", "w", encoding="utf-8") as f:
        json.dump(tf_logs, f, indent=2)

    print(f"[tf] dev_ppl={dev_ppl:.2f} test_ppl={test_ppl:.2f} train_time={tr_time:.1f}s best_ep={tf_logs['best_epoch']}")
